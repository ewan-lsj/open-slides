...
4. **Apply edits in reverse line order.**
   - Sort markers by descending `lineIndex` and process one at a time, using the StrReplace tool.
...
6. **Verify.**
   ...
   - Run `npx tsc --noEmit` if TypeScript errors are suspected. Fix any introduced errors.
