...
Use Cursor's `AskQuestion` tool when input sources are unclear.
...
If the user's original message already specifies the inputs unambiguously, skip the question and proceed. Otherwise call `AskQuestion` (multi-select) so they can pick one or more sources, and ask follow-ups (paths, slide id, prose) only as needed.
